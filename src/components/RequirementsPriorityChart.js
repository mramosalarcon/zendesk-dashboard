...
const RequirementsPriorityChart = () => {
  const [data, setData] = useState([]);
  useEffect(() => {
    fetch('http://localhost:3000/requirement-priority')
  ...
  return (
    <div style={{
      ...
      <h2 style={{ margin: '0 0 10px 0', fontSize: '18px', color: '#333' }}>
        Requerimientos por prioridad: {data.reduce((acc, item) => acc + item.ticket_count, 0)}
      </h2>
      ...
      data={{
        ...
        datasets: [{
          data: values,
          backgroundColor: ['#FFEE00', '#FF0000', '#800080', '#4BC0C0', '#fff68f'],
        }],
      }}
    />}
      </p>
...
export default RequirementsPriorityChart;
